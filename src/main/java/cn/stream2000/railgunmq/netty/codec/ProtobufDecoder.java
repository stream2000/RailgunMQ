...
public ProtobufDecoder(ProtoRouter router) {
    this.router = router;
}
...
protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
    while (in.readableBytes() > 4) { // 如果可读长度小于包头长度，退出。
        ...
        MessageStrategy handler = router.getHandler(dataType);
        // handler must not be null
        MessageStrategyContext messageStrategyContext = new MessageStrategyContext(result,handler, ctx);
